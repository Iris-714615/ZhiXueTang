...
class AliPay(object):
    ...
    def __init__(self, appid, app_notify_url, app_private_key_path,
        ...
        print(f"读取应用私钥文件: {app_private_key_path}")
        with open(self.app_private_key_path) as fp:
            key_content = fp.read()
            print(f"应用私钥内容长度: {len(key_content)}")
            self.app_private_key = RSA.importKey(key_content)
        self.alipay_public_key_path = alipay_public_key_path
        print(f"读取支付宝公钥文件: {alipay_public_key_path}")
        with open(self.alipay_public_key_path) as fp:
            key_content = fp.read()
            print(f"支付宝公钥内容长度: {len(key_content)}")
    ...
    def verify(self, data, signature):
        ...
        result = self._verify(message, signature)
